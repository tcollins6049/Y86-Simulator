...
{
    if ((address < 0) || (address > MEMSIZE - 1) || (address % 8 != 0)) { // || (address % 8 != 0)
      imem_error = true;
      return 0;
   }
   else {
...
{
   if (address < 0 || address > MEMSIZE - 1 || address % 8 != 0) {
      imem_error = true;
   }
   else {
      imem_error = false;
      uint64_t zero = Tools::getByte(value, 0);
      uint64_t one = Tools::getByte(value, 1);
      uint64_t two = Tools::getByte(value, 2);
      uint64_t three = Tools::getByte(value, 3);
      uint64_t four = Tools::getByte(value, 4);
      uint64_t five = Tools::getByte(value, 5);
      uint64_t six = Tools::getByte(value, 6);
      uint64_t seven = Tools::getByte(value, 7);
